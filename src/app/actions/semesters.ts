...
function normalizeJtInputs(
  jts: JtInput[] | undefined,
): JtInput[] | { error: string } {
  ...
  for (const jt of jts) {
    ...
    if (!/^#[0-9A-Fa-f]{6}$/.test(color)) {
      return {
        error: `Invalid color for "${name}". Use a hex color like #4f6ef7.`,
      };
    }
...
async function deactivateActiveJtFamilies(
  admin: ReturnType<typeof createAdminSupabase>,
) {
...
async function clearActiveMemberJtAssignments(
  admin: ReturnType<typeof createAdminSupabase>,
) {
  const { error } = await admin
    .from("members")
...
export async function closeActiveSemester() {
  return withServerAction("closeActiveSemester", () =>
    closeActiveSemesterImpl(),
  );
}

async function closeActiveSemesterImpl() {
  ...
  const { error } = await admin.rpc("close_semester", {
    p_semester_id: active.id,
  });
...
export async function startSemester(input: {
  ...
  return withServerAction("startSemester", () => startSemesterImpl(input));
}

async function startSemesterImpl(input: {
  name: string;
  startDate: string;
  endDate: string;
  /** Omit or empty = JT 1–6 placeholders. Custom list only when year has no JTs yet. */
  ...
  if (active) {
    return {
      success: false,
      error: "Close the current semester before starting a new one.",
    };
  }
  ...
  if (jtsToCreate) {
    ...
    if (deactivateError) {
      return {
        success: false,
        error: "Semester created but failed to deactivate prior Jiatings.",
      };
    }
    ...
    if (clearError) {
      return {
        success: false,
        error:
          "Semester created but failed to clear prior Jiating assignments.",
      };
    }
    ...
    if (jtError) {
      return {
        success: false,
        error: "Semester created but failed to create Jiatings.",
      };
    }
  }
...
export async function updateSemester(input: {
  ...
  return withServerAction("updateSemester", () => updateSemesterImpl(input));
}

async function updateSemesterImpl(input: {
  id: string;
  name: string;
  startDate: string;
  endDate: string;
  ...
  if (!semester.is_active)
    return { success: false, error: "Only the active semester can be edited." };
...
export async function createJtFamily(input: { name: string; color: string }) {
  return withServerAction("createJtFamily", () => createJtFamilyImpl(input));
}

async function createJtFamilyImpl(input: { name: string; color: string }) {
  const { error: authError } = await requireAdmin();
  ...
  if (!name)
    return { success: false, error: "Jiating name is required.", family: null };
  if (!/^#[0-9A-Fa-f]{6}$/.test(color)) {
    return {
      success: false,
      error: "Use a hex color like #4f6ef7.",
      family: null,
    };
  }
  ...
  if (!active?.year_id) {
    return {
      success: false,
      error: "No active semester with a school year.",
      family: null,
    };
  }
  ...
  if (error) {
    if (error.code === "23505") {
      return {
        success: false,
        error: `An active Jiating named "${name}" already exists.`,
        family: null,
      };
    }
...
export async function updateJtFamily(input: {
  id: string;
  name: string;
  color: string;
}) {
  return withServerAction("updateJtFamily", () => updateJtFamilyImpl(input));
}

async function updateJtFamilyImpl(input: {
  id: string;
  name: string;
  color: string;
}) {
  ...
  if (!existing || !existing.is_active) {
    return {
      success: false,
      error: "Jiating not found or inactive.",
      family: null,
    };
  }
  if (existing.year_id !== active.year_id) {
    return {
      success: false,
      error: "Can only edit Jiatings for the active school year.",
      family: null,
    };
  }
...
export async function deactivateJtFamily(id: string) {
  return withServerAction("deactivateJtFamily", () =>
    deactivateJtFamilyImpl(id),
  );
}

async function deactivateJtFamilyImpl(id: string) {
  const { error: authError } = await requireAdmin();
  if (authError) return { success: false, error: authError };
  ...
  if (existing.year_id !== active.year_id) {
    return {
      success: false,
      error: "Can only deactivate Jiatings for the active school year.",
    };
  }
...
export async function bootstrapYearPlaceholders() {
  return withServerAction("bootstrapYearPlaceholders", () =>
    bootstrapYearPlaceholdersImpl(),
  );
}

async function bootstrapYearPlaceholdersImpl() {
  const { error: authError } = await requireAdmin();
  if (authError) return { success: false, error: authError };
  ...
  if (clearError) {
    return {
      success: false,
      error: "Failed to clear prior Jiating assignments.",
    };
  }
  ...
  if (error)
    return { success: false, error: "Failed to create placeholder Jiatings." };
...
export async function listSemesters() {
  return withServerAction("listSemesters", () => listSemestersImpl());
}

async function listSemestersImpl() {
  const { supabase, error: authError } = await requireAdmin();
  if (authError)
    return { semesters: [] as SemesterListItem[], error: authError };
  ...
  const [
    ...
    fetchAllPages<{ semester_id: string; jt_family_name: string | null }>(
      (from, to) =>
        supabase
          .from("semester_summaries")
          .select("semester_id, jt_family_name")
          .range(from, to),
    ),
  ]);
  ...
  for (const event of events) {
    eventCounts.set(
      event.semester_id,
      (eventCounts.get(event.semester_id) ?? 0) + 1,
    );
  }
  ...
  for (const summary of summaries) {
    memberCounts.set(
      summary.semester_id,
      (memberCounts.get(summary.semester_id) ?? 0) + 1,
    );
    if (summary.jt_family_name) {
      const names =
        summaryJtNames.get(summary.semester_id) ?? new Set<string>();
  ...
  const enriched: SemesterListItem[] = (semesters ?? []).map((semester) => {
    ...
    if (semester.is_active) {
      const activeForYear = activeJtFamilies.filter(
        (j) => j.year_id === semester.year_id,
      );
...
export async function previewStartSemesterYear(startDate: string) {
  return withServerAction("previewStartSemesterYear", () =>
    previewStartSemesterYearImpl(startDate),
  );
}

async function previewStartSemesterYearImpl(startDate: string) {
  const { error: authError } = await requireAdmin();
  if (authError) return { yearName: "", yearHasJts: false, error: authError };

  const yearName = schoolYearFromStartDate(startDate);
  if (!yearName)
    return { yearName: "", yearHasJts: false, error: "Invalid start date." };
