...
export async function selfCheckIn(eventId: string, _semesterId?: string) {
  return withServerAction("selfCheckIn", () =>
    selfCheckInImpl(eventId, _semesterId),
  );
}

async function selfCheckInImpl(eventId: string, _semesterId?: string) {
  const supabase = await createActionSupabase();
  const {
    data: { user },
  } = await supabase.auth.getUser();
  if (!user)
    return { success: false, error: "Not authenticated.", counted: false };
  ...
  if (member.status !== "active") {
    return {
      success: false,
      error: "Your account must be active to check in.",
      counted: false,
    };
  }
  ...
  if (error) {
    ...
    return {
      success: false,
      error: "Check-in failed. Please try again or ask an officer.",
      counted: false,
    };
  }
...
export async function officerCheckIn(
  ...
  return withServerAction("officerCheckIn", () =>
    officerCheckInImpl(eventId, _semesterId, memberId),
  );
}

async function officerCheckInImpl(
  eventId: string,
  _semesterId: string,
  memberId: string,
) {
    ...
    if (linkedFamilies && linkedFamilies.length > 0) {
      ...
      if (!member.jt_family_id || !allowed.has(member.jt_family_id)) {
        return {
          success: false,
          error:
            "Only members in the participating Jiatings can be checked in to this event.",
          counted: false,
        };
      }
      ...
      return {
        success: false,
        error:
          "Only members assigned to a Jiating can be checked in to this event.",
        counted: false,
      };
    }
  }
...
export async function officerRemoveCheckIn(eventId: string, memberId: string) {
  return withServerAction("officerRemoveCheckIn", () =>
    officerRemoveCheckInImpl(eventId, memberId),
  );
}

async function officerRemoveCheckInImpl(eventId: string, memberId: string) {
  const supabase = await createActionSupabase();
  const {
    data: { user },
  } = await supabase.auth.getUser();
  ...
  if (error) {
    return {
      success: false,
      error: "Failed to remove check-in. Please try again.",
    };
  }
