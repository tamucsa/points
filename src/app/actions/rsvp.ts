...
async function assertRsvpEvent(
  ...
  if (event.check_in_type !== "rsvp_required") {
    return {
      error: "RSVP tagging is only available for RSVP events." as const,
    };
  }
  return { error: null };
}
...
export async function listEventRsvps(eventId: string) {
  return withServerAction("listEventRsvps", () => listEventRsvpsImpl(eventId));
}

async function listEventRsvpsImpl(eventId: string) {
...
export async function replaceEventRsvpCsv(eventId: string, rows: RsvpCsvRow[]) {
  return withServerAction("replaceEventRsvpCsv", () =>
    replaceEventRsvpCsvImpl(eventId, rows),
  );
}

async function replaceEventRsvpCsvImpl(eventId: string, rows: RsvpCsvRow[]) {
  const { supabase, error: authError } = await requireOfficer();
  if (authError) {
    return {
  ...
  for (let i = 0; i < inserts.length; i += chunkSize) {
    ...
    if (insertError) {
      return {
        ...
        unmatched: 0,
        errors: [
          "Failed to save RSVP rows. Previous list was cleared — please re-upload.",
        ],
      };
    }
  }
...
export async function matchEventRsvpRow(rsvpId: string, memberId: string) {
  return withServerAction("matchEventRsvpRow", () =>
    matchEventRsvpRowImpl(rsvpId, memberId),
  );
}

async function matchEventRsvpRowImpl(rsvpId: string, memberId: string) {
  const { supabase, error: authError } = await requireOfficer();
  ...
  if (existing) {
    return {
      success: false,
      error: "That member is already tagged as RSVPed for this event.",
    };
  }
...
export async function dismissEventRsvpAsGuest(rsvpId: string) {
  return withServerAction("dismissEventRsvpAsGuest", () =>
    dismissEventRsvpAsGuestImpl(rsvpId),
  );
}

async function dismissEventRsvpAsGuestImpl(rsvpId: string) {
  const { supabase, error: authError } = await requireOfficer();
  if (authError) return { success: false, error: authError };
