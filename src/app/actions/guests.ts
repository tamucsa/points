...
export async function listEventGuests(eventId: string) {
  return withServerAction("listEventGuests", () =>
    listEventGuestsImpl(eventId),
  );
}

async function listEventGuestsImpl(eventId: string) {
...
export async function replaceHowdyWeekGuestsCsv(
  ...
  return withServerAction("replaceHowdyWeekGuestsCsv", () =>
    replaceHowdyWeekGuestsCsvImpl(eventId, rows),
  );
}

async function replaceHowdyWeekGuestsCsvImpl(
  eventId: string,
  rows: HowdyWeekCsvRow[],
) {
  const {
  ...
  if (
    !isHowdyWeekCategory(event.category) ||
...
export async function clearEventGuests(eventId: string) {
  return withServerAction("clearEventGuests", () =>
    clearEventGuestsImpl(eventId),
  );
}

async function clearEventGuestsImpl(eventId: string) {
  const {
    supabase,
    member: officer,
    error: authError,
  ...
  if (
    ...
    return {
      success: false,
      error: "Guest clear is only for Howdy Week events.",
    };
  }
...
export async function matchGuestRow(guestId: string, memberId: string) {
  return withServerAction("matchGuestRow", () =>
    matchGuestRowImpl(guestId, memberId),
  );
}

async function matchGuestRowImpl(guestId: string, memberId: string) {
  const {
    supabase,
    member: officer,
    error: authError,
  ...
  if (event.publish_status !== "published") {
    return {
      success: false,
      error: "Publish this event before matching guests.",
    };
  }
  ...
  if (attendanceError) {
    ...
    return {
      success: false,
      error: "Matched guest but failed to record attendance.",
    };
  }
...
export async function linkEventGuestsByEmail(
  ...
  return withServerAction("linkEventGuestsByEmail", () =>
    linkEventGuestsByEmailImpl(supabase, memberId, email),
  );
}

async function linkEventGuestsByEmailImpl(
  supabase: SupabaseClient,
  memberId: string,
  email: string | null | undefined,
) {
...
export async function countHowdyWeekAttendanceByMemberIds(
  ...
  return withServerAction("countHowdyWeekAttendanceByMemberIds", () =>
    countHowdyWeekAttendanceByMemberIdsImpl(supabase, memberIds, semesterId),
  );
}

async function countHowdyWeekAttendanceByMemberIdsImpl(
  supabase: SupabaseClient,
  memberIds: string[],
  semesterId: string,
...
export async function listHowdyWeekGuestProspects(
  ...
  return withServerAction("listHowdyWeekGuestProspects", () =>
    listHowdyWeekGuestProspectsImpl(filters, client),
  );
}

async function listHowdyWeekGuestProspectsImpl(
  filters: HowdyWeekGuestProspectFilters = {},
  client?: SupabaseClient,
) {
  let supabase = client;
  ...
  const { data: rawRows, error } = await fetchAllPages<GuestJoinRow>(
    (from, to) => {
      let q = supabase
        .from("event_guests")
        .select(
  ...
  let prospects: HowdyWeekGuestProspect[] = [...byEmail.values()].map((agg) => {
    ...
    const lastAttended = events.reduce<string | null>((max, ev) => {
...
export async function rematchGuestEmail(email: string, memberId: string) {
  return withServerAction("rematchGuestEmail", () =>
    rematchGuestEmailImpl(email, memberId),
  );
}

async function rematchGuestEmailImpl(email: string, memberId: string) {
  const {
    supabase,
    member: officer,
    error: authError,
  ...
  if (authError || !officer) {
    return {
      success: false,
      linked: 0,
      error: authError ?? "Officer access required.",
    };
  }

  const normalized = normalizeEmail(email);
  if (!normalized)
    return { success: false, linked: 0, error: "Email is required." };
...
export async function exportHowdyWeekGuests(
  ...
  return withServerAction("exportHowdyWeekGuests", () =>
    exportHowdyWeekGuestsImpl(filters),
  );
}

async function exportHowdyWeekGuestsImpl(
  filters: HowdyWeekGuestProspectFilters = {},
) {
  const {
    supabase,
...
export async function listRecentGuestExports(limit = 5) {
  return withServerAction("listRecentGuestExports", () =>
    listRecentGuestExportsImpl(limit),
  );
}

async function listRecentGuestExportsImpl(limit = 5) {
  const { supabase, error: authError } = await requireOfficer();
  if (authError) {
    return {
      rows: [] as {
  ...
  const { data, error } = await supabase
    ...
    .select(
      "id, row_count, created_at, members!guest_export_log_exported_by_fkey(full_name)",
    )
  ...
  return {
    rows: (data ?? []).map((row) => {
      ...
      return {
        ...
        exporter_name:
          (member as { full_name?: string } | null)?.full_name ?? null,
      };
    }),
    error: null as string | null,
  };
