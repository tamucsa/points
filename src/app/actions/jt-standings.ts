...
async function requireOfficer() {
  ...
  if (!user) return { supabase, member: null, error: 'Not authenticated.' as const }
  ...
  if (!member || !['officer', 'admin'].includes(member.role)) {
    return { supabase, member: null, error: 'Officer access required.' as const }
  }
...
export async function publishJiatingStandings(eventId: string) {
  ...
  const { data: standings, error: standingsError } = await admin
    ...
    .select('jt_family, jt_color, member_count, total_points, avg_points_per_member')

  if (standingsError || !standings?.length) {
    return { success: false as const, error: 'Failed to read current Jiating standings.' }
  }
  ...
  if (snapshotError || !snapshot) {
    return { success: false as const, error: 'Failed to save standings snapshot.' }
  }
  ...
  if (rowsError) {
    ...
    return { success: false as const, error: 'Failed to save Jiating standings rows.' }
  }
