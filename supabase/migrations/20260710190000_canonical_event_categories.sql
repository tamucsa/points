-- Canonical event category names (no legacy GM / JT_Olympics labels).

UPDATE public.events SET category = 'General Meeting' WHERE category = 'GM';
UPDATE public.events SET category = 'Jiating Olympics' WHERE category = 'JT_Olympics';
...
SELECT
  ...
  COALESCE(SUM(CASE
    WHEN a.counted AND e.category IN ('Jiating Olympics', 'Mixer')
